...
from cores.polyphony            import PolyphonyGraphicsCore
from cores.wf3d                 import WF3DCore
...
class SimSoC(SoCCore):
    def __init__(self,
        ...
        **kwargs):
        ...
        # Wire-Frame 3D core
        # TODO: connect VGA output to vga pins
        # TODO: add block to self.mem_map
        if with_wf3d:
            wf3d_base_address = 0x80010000
            self.wf3d = WF3DCore(
                platform,
                prefix_cores=prefix_cores
            )
            wf3d_region = SoCRegion(origin=wf3d_base_address, size=0x1000, cached=False)
            self.bus.add_region("wf3d", wf3d_region)
            self.bus.add_master(name="wf3d", master=self.wf3d.master)
            self.bus.add_slave(name="wf3d", slave=self.wf3d.slave)

        # Polyphony --------------------------------------------------------------------------------
        # TODO: connect VGA output to vga pins
        # TODO: add block to self.mem_map
        if with_polyphony:
            pp_base_address = 0x80010000
            self.submodules.polyphony = PolyphonyGraphicsCore(
                platform,
                base_address=pp_base_address,
                prefix_cores=prefix_cores
            )
            polyphony_region = SoCRegion(origin=pp_base_address, size=0x1000, cached=False)
            self.bus.add_region("polyphony", polyphony_region)
            self.bus.add_master(name="polyphony", master=self.polyphony.master)
            self.bus.add_slave(name="polyphony", slave=self.polyphony.slave)
...
def sim_args(parser):
    ...
    # Cores
    parser.add_argument("--prefix-cores",   default=None,        help="Prefix of cores directory")

    # Wire-Frame 3D
    parser.add_argument("--with-wf3d",      action="store_true", help="Enable Wire-Frame 3D core")

    # Polyphony
    parser.add_argument("--with-polyphony", action="store_true", help="Enable Polyphony 3D core")

    # Skybox
    parser.add_argument("--with-skybox",    action="store_true", help="Enable Skybox 3D core")

    # Vortex
    parser.add_argument("--with-vortex",    action="store_true", help="Enable Vortex 3D core")
...
def main():
    ...
    soc = SimSoC(
        ...
        prefix_cores           = args.prefix_cores,
        with_wf3d              = args.with_wf3d,
        with_polyphony         = args.with_polyphony,
        with_skybox            = args.with_skybox,
        with_vortex            = args.with_vortex,
